...
const seedDB = async () => {
  try {
    ...
    const teacherRole = await Role.create({ name: 'teacher', permissions: ['create_course', 'create_post', 'schedule_meeting'] });
    const studentRole = await Role.create({ name: 'student', permissions: ['search_teachers', 'book_meeting', 'read_courses'] });
    ...
    const course1 = await Course.create({
      ...
      description: 'An advanced course focusing on Functional Dependencies, 1NF to BCNF decompositions, multi-valued dependency anomalies, query optimizer mechanics, and index optimizations.',
      duration: '6 Weeks',
      sessionsCount: 12,
      fee: 150,
